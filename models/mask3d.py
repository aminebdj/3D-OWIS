...
class Mask3D(nn.Module):
    ...
    def forward(self, x, point2segment=None, raw_coordinates=None, is_eval=False):
        ...
        return {
            ...
            'mul_queries': torch.stack(mul_queries).permute(1,0,2,3),
            # 'obj': (torch.max(self.softmax(predictions_class[-1]),dim=-1).values)*torch.stack([torch.sum(_.float().sigmoid(),dim=0)/(torch.sum((_.float().sigmoid()>0.5).float(),dim=0)+0.0001) for _ in predictions_mask[-1]])
            'obj': (torch.max(self.softmax(predictions_class[-1]),dim=-1).values)*(torch.stack([torch.mean(torch.topk(_.float(),k=10,dim=0).values,dim=0) for _ in predictions_mask[-1]]).sigmoid())
        }
    ...
    def _set_aux_loss(self, outputs_class, outputs_seg_masks):
        ...
        return [
            ...
            {"pred_logits": a, "pred_masks": b, "obj":(torch.max(self.softmax(a),dim=-1).values)*(torch.stack([torch.mean(torch.topk(_.float(),k=10,dim=0).values,dim=0) for _ in b]).sigmoid())}
